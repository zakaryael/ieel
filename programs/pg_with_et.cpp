    ...
    while(it<nstep) 
    {
        if((it % Nlearning)==0  && it != 0) {
            ...
            //update theta parameters
                //first the elegibility vector ztheta
            for(int b=0; b < 7; b++){
                theta(state, b) = lambda_theta * ztheta(state, b) - pi(state, b);
            }
            ztheta(state, action) = lambda_theta * ztheta(state, action) + delta;
            ...
            //update v parameters
                //first zv
            zv(state) = lambdav * zv(state) +  delta;
                //then v
            v(state) = v(state) + alpha_w * delta * zv(state);
