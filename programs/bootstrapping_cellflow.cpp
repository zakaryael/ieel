...
int main(int argc, char* argv[]) {
    ...
    const double learnrate = args.getreal("-lr", "--learnrate", 0.001, "Learning rate");
    ...
    const int n_bootstrapping = args.getint("-nb", "--n_bootstrapping", 1000, "number of steps for bootstrapping");
    args.check();
    mkdir(outdir);
    args.save(outdir);
    ...
    MyCol rewards; // a vector to store the last n rewards
    ...
    while(it<nstep)
    {
        if(((it % Nlearning))==0){
            ...
            if(learning_step >= n_bootstrapping){
                ...
                double delta = G + pow(gamma, n_bootstrapping - 1) * QL.get_Q(states[ind_prim], actions[ind_prim]) - Q_old;
                //new estimate q_new = q_old + alpha * (G + pow(gamma, ind)*q(current_s, current_a) - q_old)
                double Q_new = Q_old + learnrate * delta;
                if(it % Nout == 0) cout << "target: " << G + pow(gamma, n_bootstrapping - 1) * QL.get_Q(states[ind_prim], actions[ind_prim]) << " current estimate: " << Q_old << " G: " << G << " TD(n) error: " << delta << endl;
