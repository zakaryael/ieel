...
def load_data(dir, nout=400, T=600, dt=5e-4, progress=False, number_of_actions=8, number_of_states=6):
    '''load simulation data from nc files (old)'''
    ...
    frac = int(number_of_time_steps / 20)
    loaded = 1
    print("number of time steps = {}, 5 percent fraction = {}, numberOfFiles = {}".format(number_of_time_steps, frac, numberOfFiles))
    if progress: print("loading...")
    for i in range(0, number_of_time_steps, nout):
        #print("we're at i = {}".format(i))
        ...
        if progress:
            if(i >= loaded * frac):
                print("{}% loaded ".format(100 * frac * (loaded) / number_of_time_steps))
                loaded = loaded + 1
...
def print_matrix(M, sig_num=5):
    matrix = np.round(M, sig_num)
...
def reduce_data(data):
    buckledr = [3, 4, 5]
    buckledl = [9, 10, 11]
    #data_reduced['state'] = data_reduced['state'].replace(buckledr, np.array(buckledr)-3)
    #data_reduced[]
    #data_reduced['state'] = data_reduced['state'].replace(buckledl, np.array(buckledl)-6
